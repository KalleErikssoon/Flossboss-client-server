        ...
        <ul className="clinics-list">
            {clinics.map((clinicsObj, index) => (
                <li key={index} className="clinic-item">
                    <span className="clinic">{clinicsObj.name} <br /> {clinicsObj.openingHours}</span>
                    {clinicsObj?.slotsAvailable ? (
                <button className="btn btn-primary" onClick={() =>{onBookClick(clinicsObj.name, clinicsObj._id)}}>Book</button>
            ) : (
                <button type="button" className="btn" disabled aria-disabled="true">Unavailable</button>
            )}
                </li>
            ))}
        </ul>
